...
@section('content')
    <!-- event container start from here  -->
    <section class=" p-4">
        <div class="container mx-auto">
            <div class="grid grid-cols-5">
                <div class="col-span-5 sm:col-span-1">
                    <h1 class="text-center font-semibold py-2 ">Sort Events </h1>
                    ...
                    <div class="flex flex-col gap-1 py-2 px-5">
                        ...
                        @foreach ($years as $year)
                            <a class="bg-primary text-white bg-opacity-70 hover:bg-opacity-100  text-center p-1 rounded-md "
                                href="{{ route('events', ['year' => $year]) }}">{{ $year }}</a>
                        @endforeach
                    </div>
                </div>
                            ...
                            <div
                                ...
                                <div class="h-[200px] bg-red-300 overflow-hidden">
                                    <img class="h-full w-full hover:scale-125 transition-all duration-300"
                                        src="{{ asset('storage/events/' . $event->thumbnail) }}" alt="">
                                </div>
                                <div class="p-3">
                                    <a class="text-primary hover:text-secondary "
                                        href="{{ route('eventdetails', $event->id) }}">{{ $event->title }}</a>
                                    <p class="text-xs ">Date: {{ $event->event_date }}</p>
                                    <p class="text-sm text-justify mt-2">{{ $event->short_description }}</p>
                                </div>
                            </div>
                        @endforeach
