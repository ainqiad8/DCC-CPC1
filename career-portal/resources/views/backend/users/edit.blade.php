        ...
        <div class="card-body">
            <form action="{{ route('admin.users.update',$user->id) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input value="{{ old('name', $user->name) }}" class="form-control" name="name" id="name"
                        type="text" placeholder="Name" aria-label="default input example">
                    @error('name')
                ...
                <div class="form-group">
                    ...
                    <input value="{{ old('email', $user->email) }}" class="form-control" name="email" id="email"
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" name="roles" id="">
                        <option value="user" @if (old('roles', $user->role) == 'user') selected @endif>User</option>
                        <option value="admin" @if (old('roles', $user->role) == 'admin') selected @endif>Admin</option>
                    </select>
                    @error('role')
                        <div class=" my-2 alert alert-danger">{{ $message }}</div>
                    @enderror
